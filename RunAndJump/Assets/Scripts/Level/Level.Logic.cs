...
namespace RunAndJump {
	public partial class Level : MonoBehaviour {
		...
		private void Start () {
			...
			AddBackground ();
			SetGravity ();
			AudioPlayer.Instance.PlayBgm(_settings.Bgm);
		}
		
		public void SetGravity () {
			if(Physics2D.gravity.y != _settings.Gravity) {
				Physics2D.gravity = new Vector3 (0, _settings.Gravity);
			}
		}
		...
		private void AddBackground () {
			...
			background.Init(_settings.Background, playerCamera.Camera);
		}
